    ...
    if (META_ACCESS_TOKEN && META_PIXEL_ID) {
      try {
        const leadUser = {
          ...
          value: data.Value || data["Total Price"] || data.totalPrice || data.value || 1499,
          fbc: data.fbc || "",
          fbp: data.fbp || "",
...
function sendEventToMeta(eventName, u) {
  ...
  const valAmt = Number(u.value) || 1499;
  const contentName = valAmt > 1499 ? 'GHL Masterclass + Facebook Ads Upgrade' : 'GHL AI Marketing Masterclass';
